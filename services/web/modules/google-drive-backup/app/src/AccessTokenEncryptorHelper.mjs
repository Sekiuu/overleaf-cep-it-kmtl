...
const CIPHER_KEY_FILE = '/var/lib/overleaf/data/.google-drive-cipher-key'
 ...
 *   1. GOOGLE_DRIVE_CIPHER_PASSWORD env var (explicit user config)
...
function _getStableCipherPassword() {
  if (process.env.GOOGLE_DRIVE_CIPHER_PASSWORD) {
    return process.env.GOOGLE_DRIVE_CIPHER_PASSWORD
  }
  try {
...
function _getEncryptor() {
  if (!encryptorInstance) {
    const encryptorSettings =
      Settings.googleDriveBackup?.encryptor || Settings.oauthProviders?.encryptor
    if (!encryptorSettings) {
      const cipherLabel = process.env.GOOGLE_DRIVE_CIPHER_LABEL || CIPHER_LABEL
